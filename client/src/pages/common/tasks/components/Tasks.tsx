...
const Tasks:React.FC<ArgsType> = ({cid, action, data, checkDataBy, setSubNavItems, mtid}) => {
  ...
  // add Tasks
  const addTask = async ({name, value}:{name:string, value:string})=>{
    const mid = mainTaskId as unknown as string;
            ...
            <table className='w-full rounded-sm'>
              <thead>
                <tr className='text-sm font-normal'>
                  <th className='w-[5px] bg-green-200 border border-green-200'></th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  colSpan={mainTaskData && mainTaskData.customFields ? mainTaskData.customFields.length + 6 : 6}>
                      <EnterInput name='addTask' onEnter={addTask} showButton={false} 
