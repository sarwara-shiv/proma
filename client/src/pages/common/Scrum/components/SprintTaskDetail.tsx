...
const SprintTaskDetail: React.FC<ArgsType> = ({ projectId, onUpdate, task }) => {
  ...
  const renderTask = (task: Task, level = 0) => {
    ...
    return (
      <div key={task._id?.toString()} className={`relative`}>
        ...
        <div className={`border px-2 py-1 rounded-r-lg box-shadow-sm mb-4 ${level == 2 && 'border-l border-l-primary '} `}>
